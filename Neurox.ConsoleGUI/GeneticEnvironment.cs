    ...
    public class GeneticEnvironment
    {
        ...
        internal void ParseParameters(string[] args)
        {
            try
            {
                ...
                if (File.Exists(_configurationProvider.GetConfiguredFilePath("workingDirectory", "StartingIndividual")))
                {
                    var jsonContent = File.ReadAllText(_configurationProvider.GetConfiguredFilePath("workingDirectory", "StartingIndividual"));
